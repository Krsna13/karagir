...
const calcPricingOld = `  const calculatePricing = () => {
...
const calcPricingNew = `  const { min: rangeMin, max: rangeMax, avg: calculatedAverage, breakdown } = calculateEstimatedCost(
...
code = code.replace(calcPricingOld, calcPricingNew);

// 5. Remove sections 4, 5, 6, 7 block.
const sectionPattern = /\{\/\* 4️⃣ PRIMARY STRUCTURAL MATERIAL \*\/\}[\\s\\S]*?\{\/\* RIGHT COLUMN/g;
code = code.replace(sectionPattern, "{/* RIGHT COLUMN");

// 6. Insert Locality picker into Estimate card
const localitySelectHtml = `
...
`;
const estimateCardPattern = /(Calculated Area Average: <span.*?>₹\{Math.round\(calculatedAverage\).toLocaleString\("en-IN"\)\}.00<\/span>\s*<\/div>)/g;
code = code.replace(estimateCardPattern, "$1\n" + localitySelectHtml);

// 7. Update Live Technical Spec Sheet Add-ons rendering
const addonsOldPattern = /\{\/\* ⚡ 5️⃣ ACTIVE 3D MODEL ADD-ONS & FEATURES INJECTED \*\/\}[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/g;
const addonsNew = `{/* ⚡ 5️⃣ ACTIVE 3D MODEL ADD-ONS & FEATURES INJECTED */}
                  ...
                  {breakdown.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-[#1A120E] p-2 rounded-lg border border-[#2A1E17]">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <button 
                          onClick={() => removeSlot(item.role, selectedSlots.find(s => s.role === item.role && s.materialName === item.materialName)?.rateKey || "")}
...
code = code.replace(addonsOldPattern, addonsNew);

// 8. Add a 4th card "Recommended Combos" to the Blueprint Grid
const blueprintPattern = /(<div className="bg-\[\#120B08\] border border-\[\#2A1E17\] rounded-xl p-4\">\s*<h4 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-3">Hardware & Storage Accessories<\/h4>[\s\S]*?<\/div>)/g;
const recommendedCombosCard = `
...
`;
code = code.replace(blueprintPattern, "$1\n" + recommendedCombosCard);
